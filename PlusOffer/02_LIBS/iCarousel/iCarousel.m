...
- (UIView *)containView:(UIView *)view
{
    ...
    //add Swip Up gesture recogniser
    ...
    //add Swip Down gesture recogniser
    ...
    swipeDownGesture.delegate = (id <UIGestureRecognizerDelegate>)self;
...
- (void)layOutItemViews
{
    ...
    {
        // TaiT: fix iCarousel initially scrolls to currentItemIndex -1
...
- (void)didSwipe:(UITapGestureRecognizer *)swipeUpGesture
{
    NSInteger index = [self indexOfItemView:[swipeUpGesture.view.subviews lastObject]];
    if (_centerItemWhenSelected && index != self.currentItemIndex)
    {
        [self scrollToItemAtIndex:index animated:YES];
    }
    if ([_delegate respondsToSelector:@selector(carousel:didSwipeItemAtIndex:)])
    {
        [_delegate carousel:self didSwipeItemAtIndex:index];
    }
}

- (void)didSwipeDown:(UITapGestureRecognizer *)swipeDownGesture
{
    NSInteger index = [self indexOfItemView:[swipeDownGesture.view.subviews lastObject]];
    if (_centerItemWhenSelected && index != self.currentItemIndex)
    {
        [self scrollToItemAtIndex:index animated:YES];
    }
    if ([_delegate respondsToSelector:@selector(carousel:didSwipeDownItemAtIndex:)])
    {
        [_delegate carousel:self didSwipeDownItemAtIndex:index];
    }
}
