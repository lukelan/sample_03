...
-(void) layoutIfNeeded
{
    ...
    if([delegate isUserLoggedIn])
    {
        SDImageView* avatarView = [[SDImageView alloc] initWithFrame:CGRectMake(MARGIN_CELLX_GROUP, MARGIN_CELLX_GROUP, 55, 55)];
        ...
        UILabel* userName = [[UILabel alloc] initWithFrame:CGRectMake(2 * MARGIN_CELLX_GROUP + avatarView.frame.size.width, 15, 200, 20)];
        userName.backgroundColor = [UIColor clearColor];
        userName.font = [UIFont fontWithName:FONT_BOLD_NAME size:15];
        ...
        userEmail.font = [UIFont fontWithName:FONT_NAME size:15];
    ...
    else
    {
        ...
        else
        {
            ...
            UILabel *mainText = [[UILabel alloc] initWithFrame:CGRectMake(MARGIN_CELLX_GROUP, 0 , self.contentView.frame.size.width - 5*MARGIN_CELLX_GROUP - loginImage.size.width, PROFILE_CELL_NOT_LOAD_HEIGHT)];
            mainText.backgroundColor = [UIColor clearColor];
            mainText.font = [UIFont fontWithName:FONT_NAME size:13];
            mainText.text = _text;
            mainText.numberOfLines = 0;
            mainText.textAlignment = UITextAlignmentLeft;
            [self.contentView addSubview:mainText];
            
            UIButton *loginButton = [[UIButton alloc] initWithFrame:CGRectMake(self.contentView.frame.size.width - 3 *MARGIN_CELLX_GROUP - loginImage.size.width, (PROFILE_CELL_NOT_LOAD_HEIGHT - loginImage.size.height)/2, loginImage.size.width, loginImage.size.height)];
